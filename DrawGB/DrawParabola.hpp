    ...
    static LRESULT CALLBACK DGBProcParabola(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
    {
        ...
        case WM_COMMAND:
            if (LOWORD(wParam) == DGBIndexesParabola + 7) // Submit
            {
                ...
                std::string action = "PARABOLA: (" + std::to_string(temp.getX1()) + ", " + std::to_string(temp.getY1()) + "), (" + std::to_string(temp.getVertexX()) + ", " + std::to_string(temp.getVertexY()) + "), (" + std::to_string(temp.getX2()) + ", " + std::to_string(temp.getY2()) + "), Color(" + std::to_string(GetRValue(temp.getColor())) + ", " + std::to_string(GetGValue(temp.getColor())) + ", " + std::to_string(GetBValue(temp.getColor())) + ")";
                CodeAction(action);
